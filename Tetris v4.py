...
class Tetris:
    def __init__(self, master):
        ...
        GPIO.add_event_detect(buttons[3], GPIO.RISING, callback = self.rotation)
        self.master.bind('<e>', self.rotation)
        GPIO.add_event_detect(buttons[2], GPIO.RISING, callback = self.rotation)
        ...
        GPIO.add_event_detect(buttons[4], GPIO.RISING, callback = self.move)
...
#====================FPS Function===================#
    def fps(self):
        ...
        self.LvL_label = Label(self.master,
                                    ...
                                    height=5,
                                    font=('Arial Black', 12))
        self.LvL_label.grid(row=1, column=1,sticky = 'N')
...
def make(master):
    tetris = Tetris(master)
    game.mainloop()
def Start(master):
    master.title("Tetris Start")
    button = Button(master, text = 'Start',height = 15,width = 15, command = lambda : make(master))
